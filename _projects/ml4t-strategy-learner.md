...
*Cumulative portfolio value for the learned strategy versus the manual benchmark across the in-sample period. The learned strategy consistently navigates volatile periods with greater agility. Chart generated from backtesting experiments on JPMorgan Chase (JPM) stock data, CS 7646 course framework.*

> **Image source:** Original experiment output generated by the author using the CS 7646: Machine Learning for Trading backtesting framework (Georgia Tech OMSCS). Data: historical JPM equity prices, 2008–2010.
...
*Performance of both strategies as market impact parameter λ varies from 0.001 to 0.009. The learned strategy's advantage is largest at low friction and degrades faster than the manual strategy as impact increases. Original experiment output generated by the author.*

> **Image source:** Author's backtesting experiments using the CS 7646 framework. Impact model follows Almgren, R. & Chriss, N. (2001). Optimal execution of portfolio transactions. *Journal of Risk, 3*, 5–39.
...
*The manual strategy's equity curve exhibits cleaner entry and exit signals but misses the intra-period opportunities that the learned strategy captures under low-friction conditions. Original experiment output generated by the author.*

> **Image source:** Author's backtesting results using the CS 7646: Machine Learning for Trading framework (Georgia Tech OMSCS). Technical indicators based on Wilder, J.W. (1978). *New Concepts in Technical Trading Systems*. Trend Research.
